                    ...
                    option:(NIMKitInfoFetchOption *)option
{
    ...
    //游客登录假消息头像
    if (XLNIMService.shared.isVisitorLogin) {
        XLStartConfigModel *model = XLStartConfigManager.shared.startConfigModel;
        if ([info.showName isEqualToString:model.vNickname]) {
            info.avatarUrlString = model.vAvatar;
        }
        if ([info.showName isEqualToString:model.cNickname]) {
            info.avatarUrlString = model.cAvatar;
        }
        if ([info.showName caseInsensitiveCompare:XLNIMService.shared.visitorAccount] == NSOrderedSame) {
            //为游客添加URL头像时使用
        }
    }
    return info;
...
- (UIImage *)defaultTeamAvatar
{
    ...
    {
        _defaultTeamAvatar = [UIImage imageWithColor:[UIColor colorWithString:COLOREDECED] size:CGSizeMake(1, 1)];
    }
...
- (UIImage *)defaultUserAvatar
{
    ...
    {
        _defaultUserAvatar = [UIImage nim_imageInKit:@"session_avatar"];
    }
