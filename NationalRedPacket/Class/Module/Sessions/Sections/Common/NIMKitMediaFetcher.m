...
- (void)fetchPhotoFromLibrary:(NIMKitLibraryFetchResult)result
{
    ...
    [self setUpPhotoLibrary:^(UIImagePickerController * _Nullable picker) {
        if (picker && weakSelf) {
...
- (void)setUpPhotoLibrary:(void(^)(UIImagePickerController * _Nullable picker)) handler
{
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status){
        dispatch_async(dispatch_get_main_queue(), ^{
            if (status == PHAuthorizationStatusRestricted || status == PHAuthorizationStatusDenied) {
                [[[UIAlertView alloc] initWithTitle:@"相册访问权限关闭了"
                                            message:@"请在iPhone的“设置-隐私-照片”中打开神段子的相册访问权限"
                                           delegate:nil
                                  cancelButtonTitle:@"好"
            ...
            if (status == PHAuthorizationStatusAuthorized) {
                ...
                if(handler) handler(imagePicker);
            }
        });
    }];
}
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeMovie]) {
        ...
        [[PHImageManager defaultManager] requestImageDataForAsset:asset options:options resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
            ...
            if ([isError boolValue] || [isCloud boolValue] || ! imageData) {
                self.libraryResultHandler(nil, nil, PHAssetMediaTypeImage);                    return;
            }
            ...
            self.libraryResultHandler(nil, tempPath, PHAssetMediaTypeImage);
