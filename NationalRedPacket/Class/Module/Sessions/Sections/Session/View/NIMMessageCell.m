...
- (void)makeComponents
{
    ...
    //headerView
    _headImageView = [[NIMAvatarImageView alloc] initWithFrame:CGRectMake(0, 0, adaptWidth750(80), adaptWidth750(80))];
...
- (void)onCatchEvent:(NIMKitEvent *)event{
    if ([self.delegate respondsToSelector:@selector(onTapCell:event:)]) {
        [self.delegate onTapCell:self event:event];
    }
}
