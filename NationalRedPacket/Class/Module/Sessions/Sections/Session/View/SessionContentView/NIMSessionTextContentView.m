...
- (void)refresh:(NIMMessageModel *)data{
    ...
    //会话不是官方账号
    if (![officialIdArray containsObject:data.message.session.sessionId]) {
        return;
    }
    //消息不是官方账号发出的
    if (!(data.message.isOutgoingMsg && [officialIdArray containsObject:NIMSDK.sharedSDK.loginManager.currentAccount])) {
        return;
    }
    //官方账号发的消息才去匹配管理规范链接
    NSString *linkStr = @"《神段子社区管理规范》";
    if ([_textLabel.text containsString:linkStr]) {
        [self.textLabel addCustomLink:URL_MANAGERMENT_SEPECIFICATION forRange:[self.textLabel.text rangeOfString:linkStr] linkColor:data.message.isOutgoingMsg ? [UIColor blueColor] : [UIColor colorWithString:COLORFE6969]];
    };
}
