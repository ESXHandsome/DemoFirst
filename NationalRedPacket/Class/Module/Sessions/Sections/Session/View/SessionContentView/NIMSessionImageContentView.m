...
- (void)refresh:(NIMMessageModel *)data
{
    ...
    if ([imageObject.path containsString:@".GIF"] || [imageObject.path containsString:@".gif"]) {
        FLAnimatedImage *image = [FLAnimatedImage animatedImageWithGIFData:[NSData dataWithContentsOfFile:imageObject.path]];
        if (image) {
            self.imageView.animatedImage = image;
        } else {
            [self.imageView sd_setImageWithURL:[NSURL URLWithString:imageObject.url]];
        }
    } else {
        self.imageView.image = [UIImage imageWithContentsOfFile:imageObject.thumbPath];
    }
