...
//    [_toolBar.voiceBtn addTarget:self action:@selector(onTouchVoiceBtn:) forControlEvents:UIControlEventTouchUpInside];
    ...
    //设置草稿
    NSString *draftStr = [NIMKitUtil draftInSession:self.session];
    if (draftStr && draftStr.length != 0 ){
        self.toolBar.contentText = draftStr;
    }
}
...
- (void)textViewDidChange
{
    ...
    [NIMKitUtil saveDraft:self.toolBar.contentText inSession:self.session];
}
