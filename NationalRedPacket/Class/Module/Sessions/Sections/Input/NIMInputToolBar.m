...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        _top = [[UIView alloc] initWithFrame:CGRectZero];
        _top.backgroundColor = [UIColor colorWithString:COLORE6E6E6];
...
//        [_voiceBtn sizeToFit];
        ...
        [_emoticonBtn setImage:[UIImage nim_imageInKit:@"chat_expression_icon"] forState:UIControlStateNormal];
        [_emoticonBtn setImage:[UIImage nim_imageInKit:@"chat_expression_h"] forState:UIControlStateHighlighted];
        [_emoticonBtn sizeToFit];
        
        
        _moreMediaBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [_moreMediaBtn setImage:[UIImage nim_imageInKit:@"chat_add_icon"] forState:UIControlStateNormal];
        [_moreMediaBtn setImage:[UIImage nim_imageInKit:@"chat_add_h"] forState:UIControlStateHighlighted];
...
//        [_inputTextBkgImage setImage:[[UIImage nim_imageInKit:@"icon_input_text_bg"] resizableImageWithCapInsets:UIEdgeInsetsMake(15,80,15,80) resizingMode:UIImageResizingModeStretch]];
        ...
        _inputTextView.font = [UIFont systemFontOfSize:adaptFontSize(30)];
        _inputTextView.maxNumberOfLines = 5;
        _inputTextView.minNumberOfLines = 1;
        _inputTextView.textColor = [UIColor blackColor];
        _inputTextView.backgroundColor = [UIColor colorWithString:COLORF7F7F7];
        _inputTextView.nim_size = [_inputTextView intrinsicContentSize];
        _inputTextView.textViewDelegate = self;
        _inputTextView.returnKeyType = UIReturnKeySend;
        _inputTextView.layer.borderWidth = adaptHeight1334(1);
        _inputTextView.layer.borderColor = [UIColor colorWithString:COLORE8E8E8].CGColor;
        _inputTextView.layer.cornerRadius = 2;
        _inputTextView.layer.masksToBounds = YES;
        
        _sep = [[UIView alloc] initWithFrame:CGRectZero];
        _sep.backgroundColor = [UIColor colorWithString:COLORE6E6E6];
...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    if (self.status == NIMInputStatusAudio) {
        ...
        //得到 ToolBar 自身高度
        viewHeight = self.inputTextView.nim_height + 2 * self.textViewPadding;
    }
...
- (void)layoutSubviews{
    ...
    for (NSNumber *type in self.types) {
        ...
        if (type.integerValue == 0) {
            view.nim_left = left + 0;
        } else {
            view.nim_left = left + self.spacing;
        }
        view.nim_centerY = self.nim_height * .5f;
        //保证表情和更多按钮在输入框变高时，Frame保持不变
        if ([view isEqual:self.emoticonBtn] || [view isEqual:self.moreMediaBtn]) {
            view.nim_centerY = self.nim_height - TAB_BAR_HEIGHT/2.0;
        }
        left = view.nim_right;
    }
    ...
    //上方
    CGFloat sepHeight = adaptHeight1334(1);
    _top.nim_size     = CGSizeMake(self.nim_width, sepHeight);
    _top.nim_top   = 0;
...
{
    [self.emoticonBtn setImage:selected?[UIImage nim_imageInKit:@"chat_expression_icon"]:[UIImage nim_imageInKit:@"chat_expression_switch"] forState:UIControlStateNormal];
    [self.emoticonBtn setImage:selected?[UIImage nim_imageInKit:@"chat_expression_h"]:[UIImage nim_imageInKit:@"chat_expression_switch_h"] forState:UIControlStateHighlighted];
}
...
{
    CGFloat toolBarHeight = height + 0 * self.spacing;
...
{
    self.nim_height = height + 0 * self.spacing + 2 * self.textViewPadding;
