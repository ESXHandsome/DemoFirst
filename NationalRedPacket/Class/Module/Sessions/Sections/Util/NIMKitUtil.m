...
//    }
    ...
    {
        result = [[NSString alloc] initWithFormat:@"%@%02ld:%02d",result,(long)hour,(int)msgDateComponents.minute];
    }
    else if(isSameMonth && (nowDateComponents.day == (msgDateComponents.day+1)))//昨天
    {
        result = showDetail?  [[NSString alloc] initWithFormat:@"昨天%@ %02ld:%02d",result,(long)hour,(int)msgDateComponents.minute] : @"昨天";
    }
...
//    }
    else//显示日期
    {
        ...
        result = showDetail? [day stringByAppendingFormat:@"%@ %02ld:%02d",result,(long)hour,(int)msgDateComponents.minute]:day;
    }
    return result;
}
...
+ (NSString *)draftInSession:(NIMSession *)session {
    
    return [NSUserDefaults.standardUserDefaults objectForKey:[NSString stringWithFormat:@"%@_%@", session.sessionId, XLUserDefaultsSessionDraftKey]];
}

+ (void)saveDraft:(NSString *)draft inSession:(NIMSession *)session {
    
    [NSUserDefaults.standardUserDefaults setObject:draft forKey:[NSString stringWithFormat:@"%@_%@", session.sessionId, XLUserDefaultsSessionDraftKey]];
