...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.titleLabel.font = [UIFont systemFontOfSize:adaptFontSize(26)];
        self.titleEdgeInsets = UIEdgeInsetsMake(0, adaptWidth750(12), 0, 0);
        [self setTitleColor:[UIColor colorWithString:COLORFF6767] forState:UIControlStateSelected];
        [self setTitleColor:[UIColor colorWithString:COLORBDBDBD] forState:UIControlStateNormal];
...
- (void)buttonAction:(UIButton *)sender {
    ...
    if (!XLLoginManager.shared.isAccountLogined) {
        [LoginViewController showLoginVCFromSource:self.loginSourceType];
        return;
    }
    
    if (self.animationType == LikeAnimationButtonTypeTread) {
        if (self.feedModel && self.feedModel.isPraise) {
            [MBProgressHUD showError:@"您已顶过"];
            return;
        }
    } else {
        if (self.feedModel && self.feedModel.isTread) {
            [MBProgressHUD showError:@"您已踩过"];
            return;
        }
    }
